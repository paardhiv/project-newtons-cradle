...
function draw() {
  ...
  textSize(20);
  text("Press all the 4 direction keys one by one and watch the different ways of Newton's Cradle",6,100);
...
function keyPressed(){
	...
	if(keyCode === DOWN_ARROW){
		Matter.Body.applyForce(bob2.body,bob2.body.position,{x: 55, y:-50});
	}
	if(keyCode === LEFT_ARROW){
		Matter.Body.applyForce(bob1.body,bob1.body.position,{x:-60, y:50});
		Matter.Body.applyForce(bob5.body,bob5.body.position,{x:-60, y:50});
	}
	if(keyCode === RIGHT_ARROW){
		Matter.Body.applyForce(bob1.body,bob1.body.position,{x:60, y:-50});
		Matter.Body.applyForce(bob5.body,bob5.body.position,{x:60, y:-50});
		Matter.Body.applyForce(bob4.body,bob4.body.position,{x:60, y:-50});
	}
